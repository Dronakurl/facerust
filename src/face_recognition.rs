...
use opencv::{
    core::{Mat, Point, Rect2i, Scalar, Size, Ptr},
...
impl FaceRecognition {
    pub fn new(
        ...
        let detector = FaceDetectorYN::create(
            fd_path,
            "",
            Size::new(320, 320), // Use fixed input size
            SCORE_THRESHOLD,
            NMS_THRESHOLD,
            TOP_K,
            ...
            if person_path.is_dir() {
                ...
                for img_entry in std::fs::read_dir(&person_path)? {
                    ...
                    if !img_path.is_dir() {
                        ...
                        debug!("Loading image: {} for person {}", img_path.display(), person_name);
                        ...
                        for detected_face in detected_faces {
                            person_features.push(detected_face.feature);
                        }

                        // Create visualized version if requested
                        if visualize {
                            let stem = img_path.file_stem().and_then(|s| s.to_str()).unwrap_or("image");
                            let extension = img_path.extension().and_then(|e| e.to_str()).unwrap_or("jpg");
                            ...
                            for face in faces {
                                if let Ok(bbox) = face.bbox() {
                                    self.visualize_face(&mut vis_img, bbox)?;
                                }
                            }
                            
                            let _ = imwrite(visualize_path.to_str().unwrap(), &vis_img, &opencv::core::Vector::new());
                        }
                    }
                }
    ...
    pub async fn start_watching(&self, _check_interval_seconds: u64) -> Result<()> {
        let db_path = {
            ...
            path_guard.clone().ok_or(FaceRecognitionError::DatabaseNotLoaded)?
        };

        if self.watcher_running.load(std::sync::atomic::Ordering::Relaxed) {
        ...
        *watcher_guard = Some(watcher);
        drop(watcher_guard);
        self.watcher_running.store(true, std::sync::atomic::Ordering::Relaxed);
    ...
    async fn extract_features(&mut self, mut frame: Mat) -> Result<Vec<DetectedFace>> {
        ...
        // Resize frame to match detector input size (320x320)
        let detector_input_size = Size::new(320, 320);
        let mut detection_frame = Mat::default();
        opencv::imgproc::resize(&frame, &mut detection_frame, detector_input_size, 0.0, 0.0, opencv::imgproc::INTER_LINEAR)?;

        // Detect faces on the resized frame
        let mut faces = Mat::default();
        match self.detector.detect(&detection_frame, &mut faces) {
            Ok(_) => {},
            Err(e) => {
        ...
        for i in 0..faces.rows() {
            ...
            let scale_x = frame.cols() as f32 / detector_input_size.width as f32;
            let scale_y = frame.rows() as f32 / detector_input_size.height as f32;
            
            // Create scaled face detection matrix for alignment
            let mut scaled_face = face_row.try_clone()?;
            for j in 0..4 { // First 4 values are x1, y1, x2, y2
                let val = *scaled_face.at_2d::<f32>(0, j)?;
                let scaled_val = if j % 2 == 0 { val * scale_x } else { val * scale_y };
                *scaled_face.at_2d_mut::<f32>(0, j)? = scaled_val;
            }
            
            // Align and crop face using original frame and scaled coordinates
            let mut aligned_img = Mat::default();
            match self.face_recognizer.align_crop(&frame, &scaled_face, &mut aligned_img) {
                Ok(_) => {},
                Err(e) => {
            ...
            detected_faces.push(DetectedFace::new(
                "Unknown".to_string(),
                scaled_face,
                feature,
                original_size,
    ...
    fn resize_frame(&self, frame: &mut Mat, keep_aspect_ratio: bool) -> Result<()> {
        ...
        if keep_aspect_ratio {
            if cols > self.max_size || rows > self.max_size {
                ...
                let new_size = Size::new((cols as f64 * scale) as i32, (rows as f64 * scale) as i32);
                let mut resized = Mat::default();
                opencv::imgproc::resize(frame, &mut resized, new_size, 0.0, 0.0, opencv::imgproc::INTER_LINEAR)?;
                *frame = resized;
            }
        } else {
    ...
    async fn find_best_match(&mut self, face_feature: &Mat, threshold: f32) -> Result<MatchResults> {
        ...
        for (person_name, features) in features_map.iter() {
            for feature in features {
                let score = self.face_recognizer.match_(face_feature, feature, 0)? as f32; // FR_COSINE = 0
                results.push(MatchResult::new(person_name.clone(), score));
                
                debug!("Person {}, score: {}", person_name, score);
    ...
    pub async fn run(&mut self, frame: &mut Mat, threshold: f32, visualize: bool) -> Result<Vec<MatchResult>> {
        let frame_for_detection = if visualize { frame.clone() } else { frame.clone() };
        ...
        for (i, face) in detected_faces.iter().enumerate() {
            ...
            if visualize {
                if let Ok(bbox) = face.bbox() {
                    ...
                    self.annotate_with_name(frame, &face, &best.name)?;
                }
            }
        }
    ...
    pub async fn run_one_face(&mut self, mut frame: Mat, threshold: f32, visualize: bool) -> Result<MatchResult> {
    ...
    fn annotate_with_name(&self, frame: &mut Mat, face: &DetectedFace, name: &str) -> Result<()> {
        ...
        put_text(frame, name, text_pos, font_face, font_scale, text_color, thickness, LINE_8, false)?;
